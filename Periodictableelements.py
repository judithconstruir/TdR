...
#####
def total_energy_of_electrons_in_eV(element):
    n = element.energetic_levels

    total_energy_electrons = 0
    for i in n:
        total_energy_electrons += element.Z ** 2 / i **2 * 13.6

    return
#####

def numbers_of_oxidation(formula):
    general_compound_oxidation_number = 0
    elements_atoms, charge = elements_in_formula(formula)
    oxidation_number_by_element = {}


    if classification_of_compounds(formula) == "simple":
        
        if charge == 0 or charge == -9:
            oxidation_number_by_element[elements_atoms[key].X] = 0
        
        else:
            oxidation_number_by_element[elements_atoms[key].X] = charge
            general_compound_oxidation_number = charge
               

            element_oxidation_number = 0

            for key in elements_atoms:
                print((elements_atoms[key].name).capital(),":", element_oxidation_number)
    ...
    if classification_of_compounds(formula) == "binary":
        compound_type = name_formula(formula, "type")

        if "O" in elements_atoms:
            #By default, the oxidation number of oxygen is -2, except for peroxides then is -1, and when is combined with fluorine, then it's +2
            if F in elements_atoms:
                oxidation_number_by_element["O"] = +2
                oxidation_number_by_element["F"] = F.ox_n[0]
            
            elif compound_type == "peroxide":
                #The element has to be alkaline or alkalinoterri in a peroxide
                #Peroxides always have two oxygen atoms
                if elements_atoms["O"] == 2:

                    for key in elements_atoms:
                        element_numbers_of_oxidation = get_element_object(elements_atoms[key]).ox_n
                        quantity_of_numbers_oxidation = len(element_numbers_of_oxidation)


                        if quantity_of_numbers_oxidation == 1 and element_numbers_of_oxidation[0] * elements_atoms[key] == 4:
                            oxidation_number_by_element[key] = get_element_object(elements_atoms[key]).ox_n

                        elif elements_atoms[key] != "O":
                            return "It is not a peroxide"
                        
                    oxidation_number_by_element["O"] = -1

            elif compound_type == "oxide":
                oxidation_number_by_element["O"] = -2

        #водворода 
        elif "H" in elements_atoms:
            
            #all hydrogen have +1 as a oxidation number, except metal hydrides, then it has -1

            if compound_type == "metal hydride":
                oxidation_number_by_element["H"] = 1
    ...
    return oxidation_number_by_element, general_compound_oxidation_number
...
#
#Halogenated derived
#Functional groups. Oxygenated compounds 
#Nitrogenous compounds 


#Depending on the number of atoms of carbon, organic nomenclature has the following prefixes:
organic_prefixes = {1:"meth",
                    2:"eth",
                    3:"prop",
                    4:"but",
                    5:"pent",
...
#Depending on functional groups that the contains
functional_groups = {1:"Carboxylic acids",
          2:"Esters",
          3:"Amides",
          4:"Nitriles",
          5:"Aldehydes",
...
def calculate_electrons_holes_valence(element):
    ...
    elif element.group <= 10:
        return 10 - element.group
        
    elif element.group <= 18:
        return 18 - element.group
        
    else:
        return "Element group not valid."

def organic_names(formula):
    
    atoms = elements_in_formula(formula, "atoms")
    group_compound = ""

    #Check if is an organic compound
    if "C" not in atoms:
        return "Try inorganic compound"
    
    if "C" in atoms and "H" in atoms:
        group_compound = "Hydrocarbon"
    
    #hydrocarbon_type "saturated"
    #hydrocarbon_type "unsaturated"
    #-       =         -=
...
#for element in elements_list:
#    print(element.name, (element.A-element.Z) / element.Z)
...
#calculate_electric_force(proton_charge, proton_charge,1.6e-15)) # 90 N 



def print_all_elements_configuration():
    for element in elements_list:
        print(element.name, element.electronic_configuration())

#print_all_elements_configuration()
#print(Li.name, Li.electronic_configuration())
#print(Be.name, Be.electronic_configuration())
#print(B.name, B.electronic_configuration())
